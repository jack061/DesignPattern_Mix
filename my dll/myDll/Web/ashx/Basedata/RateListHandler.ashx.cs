...
namespace RM.Web.ashx.Basedata
{
    ...
    /// </summary>
    public class RateListHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        private string getList(HttpContext context)
        {
            ...
            DataTable dt = basedata_idao.GetRatePage(SqlWhere, IList_param, page, row, ref count);
        ...
        private string Save(HttpContext context)
        {
            ...
            string ratedate = context.Request.QueryString["ratedate"];
            ...
            if (methods == "1")
            {
                sql = "Insert into brate( code, name, rate, ratedate, des, status, createman, lastmod) values('" + code + "', '" + name + "','" + rate + "',  '" + ratedate + "', '" + des + "', '" + status + "','" + RequestSession.GetSessionUser().UserId + "', '" + RequestSession.GetSessionUser().UserId + "')";
            }
            else if (methods == "0")
            {
                sql = "Update brate set  name='" + name + "',rate='" + rate + "', ratedate='" + ratedate + "', des='" + des + "',  lastmod='" + RequestSession.GetSessionUser().UserId + "', lastmoddate=getdate(), status='" + status + "' where  code = '" + code + "'";
            }
        ...
        {
            string code = context.Request.QueryString["code"];
            string sql = "delete from brate where code='" + code + "'";
