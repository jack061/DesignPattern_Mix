...
function FixedTableHeader(gv, scrollHeight) {
    ...
    var minwidth = $(gv).css('width');

    $(gv).wrap("<div id='FixedTable' style='min-width:"+minwidth+";height:" + scrollHeight + "px;overflow-y: auto; overflow-x: hidden;scrollbar-face-color: #e4e4e4;scrollbar-heightlight-color: #f0f0f0;scrollbar-3dlight-color: #d6d6d6;scrollbar-arrow-color: #240024;scrollbar-darkshadow-color: #d6d6d6; padding: 0;margin: 0;'></div>");
    ...
    $(gvn).css('min-width', minwidth);
}
/**.div-body 自应表格高度**/
function divresize(height, scrollHeight) {
    //resizeU();
    $(window).resize(resizeU);
    function resizeU() {
        $(".div-body").css("height", $(window).height() - height);
        if ($("#FixedTable") != null && scrollHeight !=null) {
            $("#FixedTable").css("height", $(window).height() - scrollHeight);
            //$("#FixedTable").css("width", $(".grid").width());
        }
    }
}
//Tab标签切换
