$(function () {
    ...
    if (isBrowse == 'true') {
        ...
        //$('input').css({ "border": "0px", "background": "none" });
    }
...
function uploadFile() {
    $("#form1").ajaxSubmit({
        url: "/ashx/Basedata/BFactory.ashx?action=upload&customerStatus=0",
        type: "post",
        success: function (data) {
            if (data == "error") {
                ...
                var str = data.split(':');
                $("#boxorderUrl").textbox('setText', str[0]); //str[1]-文件名称
                $("input[name=boxorderUrl]").val(str[0]);
                $("#boxorderName").textbox('setText', str[1]); //str[1]-文件名称
