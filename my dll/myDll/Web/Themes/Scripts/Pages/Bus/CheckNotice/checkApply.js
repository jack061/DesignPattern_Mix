...
//添加
function add() {
    window.top.addNewTab("海运发货出库指令-新增", '/Bus/CheckOutNotice/checkApplyForm.aspx?action=add&isBooking=' + isBooking, '');
}
//修改
function edit() {
    var no = '';
    ...
    window.top.addNewTab("海运发货出库指令-修改", '/Bus/CheckOutNotice/checkApplyForm.aspx?action=edit&checkId=' + no, '');
}
...
function browse() {
    ...
    no = row.checkId;
    if (row != undefined) {
        window.top.addNewTab("海运发货出库指令-查看", '/Bus/CheckOutNotice/checkApplyForm.aspx?action=view&checkId=' + no, '');
    }
}
...
function addProductlist() {
    ...
    $('#productname').datagrid({
        url: '/ashx/CheckNotice/CheckApplyData.ashx?module=shipproduct&checkId=' + checkCode,
        row: 10,
        page:1,
        singleSelect: true,
        pagination: false,
