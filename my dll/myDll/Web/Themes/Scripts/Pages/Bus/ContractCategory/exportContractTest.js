...
$(function () {
    ...
    if (contractNo == "") {
        ...
        $("#form1").form('load', '/ashx/Contract/reviewContractData.ashx?module=LoadDataByExportTemp&templateno=' + templateno);
    }
        //如果为复制或者创建关联
    else {
        if (isContact == "true" ) {
    ...
    //else {
    //    $("#form1").form('load', '/ashx/Contract/reviewContractData.ashx?module=LoadApp&contractNo=' + contractNo + "&isCopy=" + isCopy + "&isattachFrame=" + isattachFrame);
    //}

    //$("#pricement1per").numberbox('setValue', contractInfo.pricement1per);
    //绑定combobox 
    bindCombobox();
    //绑定产品列表
    if (isInspect == "true") {//加载商检合同产品列表
        $('#tt').tabs('getTab', '文本条款').panel('options').tab.hide();
        $('#tt').tabs('getTab', '审核日志').panel('options').tab.hide();
        popInspectProduct();
    }
    //绑定产品列表
...
var editTemp = function () {
    ...
    $("#tempDiv").dialog({
        ...
        buttons: [{
            ...
            handler: function () {
                //调用子页面方法，保存编辑模板
                document.getElementById("tempIframe").conten
                tWindow.save1();
...
var SaveDataToDB = function (status) {
    ...
    if (isContact == "true" || isContactEdit == "true") {
        ...
        url = '/ashx/Contract/contractOperater.ashx?module=addFrameAttachContractTest&status=' + status + "&mainContractNo=" + mainContractNo + '&frameCotactContractNo=' + frameCotactContractNo + '&isCopy=' + isCopy + '&isEdit=' + isEdit + '&isConManage=' + isConManage + '&conManage_contractNo=' + contractNo;//添加框架合同附件
    }
    else {
    ...
    //if (isattachFrame == "true") {
    //    url = '/ashx/Contract/contractOperater.ashx?module=addFrameAttachContractTest&status=' + status+"&mainContractNo="+mainContractNo;
    //}
    //else {
    //    url = '/ashx/Contract/contractOperater.ashx?module=addContractTest&status=' + status;
...
var bindUI = function () {
    ...
    ////说明是新建合同,或者复制合同,创建框架合同附件
   
    //if (isnew == "True" || isCopy == "true" || isattachFrame == "true") {
    //    $('#contractNo').textbox('setValue', '自动编号');
    //    $('#contractNo').textbox('readonly', true);
    //}
    //else {
    ...
    //    $("input[name='frameContract'][value='否']").attr("checked", true);//是否为框架合同默认为否
    //    $(':radio').attr('disabled', true);
    //}
    ////说明为查看
    //if (isBrowse == "true") {
    ...
    var v = contractInfo.pricement1per;
    if (parseInt(v) == 100) {
...
function popInspectProduct() {
    $('#htcplist').datagrid({
        ...
        columns: [[
		            { field: 'pcode', title: 'SAP编号', width: 100 },
		            { field: 'pname', title: 'SAP名称', width: 100, align: 'center' },
                   { field: 'inspectingCount', title: '商检总数量', width: 100, align: 'center' },
                    { field: 'thisinsQuantity', title: '本次商检数量', width: 100, align: 'center' },
                    { field: 'qunit', title: '数量单位', width: 100 },
                    { field: 'price', title: '价格', width: 100, editor: { type: 'numberbox', options: { precision: 2 } } },
                    { field: 'priceUnit', title: '价格单位', width: 100 },
                              {
