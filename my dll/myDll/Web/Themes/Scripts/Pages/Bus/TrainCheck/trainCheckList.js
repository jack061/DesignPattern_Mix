...
function printwaybill() {
    ...
    if (row.blockTrain == "1") {
        $.messager.confirm('确认对话框', '是班列，确定要打印吗？', function (r) {
            if (r) {
                window.top.addNewTab("铁路付费代码打印", "/Bus/TrainCheckOut/trainPrintPayCode.aspx?createDateTag=" + row.createDateTag);
            }
        });
        return;
    }
...
var sendNotice = function () {
    var row = $("#maingrid").datagrid('getSelected');
    if (row == undefined || row.savestatus == "1") {
        $.messager.alert('提示', '请选择发货未完成数据！', 'info');
    }
    else {
        if (row.savestatus=="0"||row.savestatus=="1") {
            window.top.addNewTab("铁路发货出库指令-新增", "/Bus/TrainCheckOut/trainCheckFormTest.aspx?contractNo=" + row.contractNo + "&isnew=false" + "&createDateTag=" + row.createDateTag+'&noticeTag='+row.noticeTag);
        }
        else {
            window.top.addNewTab("铁路发货出库指令-新增", "/Bus/TrainCheckOut/trainCheckFormTest.aspx?contractNo=" + row.contractNo + "&isnew=true" + "&createDateTag=" + row.createDateTag + '&noticeTag=' + row.noticeTag);
        }
...
var browse = function () {
    ...
    //if (row != undefined) {
    //    //checkId = row.checkId;
    //    window.top.addNewTab("铁路发货出库指令-查看", "/Bus/TrainCheckOut/trainCheckFormTest.aspx?contractNo=" + row.contractNo + "&isBrowse=true" + "&createDateTag=" + row.createDateTag);
    //} else {
    //    $.messager.alert('提示', '请选择一行数据！', 'info');
    //}
}
