$(function () {
    $('#maingrid').datagrid({
        ...
        nowrap: false,
        fit: true,
        url: '/ashx/Contract/contractData.ashx?module=getServiceListByRoles',
        frozenColumns: [[
            ...
            { field: 'partyC', title: '丙方', width: 100 },
            { field: 'partyD', title: '丁方', width: 100 },
...
function browse() {
    ...
    no = row.contractNo;
    if (row != undefined) {
        window.top.addNewTab("物流合同-查看", '/Bus/ContractCategory/ServiceForm.aspx?contractNo=' + row.contractNo + '&isBrowse=true', '');
    }
}
//预览
function preview() {
    ...
    var row = $("#maingrid").datagrid('getSelected');
    if (row != undefined) {
        window.top.addNewTab("物流合同-预览", '/Bus/ContractCategory/previewServiceContract.aspx?&contractNo=' + row.contractNo + '&language=中文', '');
    }
}
//查找
function doSearch() {
