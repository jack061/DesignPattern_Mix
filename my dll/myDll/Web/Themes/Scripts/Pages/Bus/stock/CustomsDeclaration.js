...
$(document).ready(function () {
    $('#tt').datagrid({
        ...
        pageList: [10, 15, 30],
        singleSelect: true,
        url: 'CustomsDeclaration.ashx?action=GetCheckContractList',
        columns: [[
        ...
        toolbar: [{
            ...
            handler: function () {
                ...
                if (row != undefined) {
                    ...
                    if (row.transport === '铁路')
                        ...
                        window.top.addTab('海运报关单校对', '/Bus/StockManage/CustomsDeclarationForm.aspx?contractNo=' + row.contractNo + "&isBrowse=false");
                } else {
            ...
            handler: function () {
                ...
                if (row != undefined) {
                    if (row.transport === '铁路')
                        ...
                        window.top.addTab('报关单校对-查看', '/Bus/StockManage/CustomsDeclarationForm.aspx?contractNo=' + row.contractNo + "&isBrowse=true");
                } else {
                    $.messager.alert('提示', '请选择一行数据！', 'info');
                }
            }
            ...
            handler: function () {
                ...
                if (row != undefined) {
                    ...
                    if (row.transport === '铁路')
                        window.top.addTab('铁路报关单校对税务发票', '/Bus/StockManage/CustomsDeclarationRail.aspx?type=CustomsDeclarationPrint&contractNo=' + row.contractNo + "&isBrowse=true");
                    else
                        window.top.addNewTab('税务发票', '/Bus/StockManage/CustomsDeclarationPrintForm.aspx?type=other&no=' + no + "&language=" + language + "&choice=" + language, "");
                } else {
                    $.messager.alert('提示', '请选择一行数据！', 'info');
                }
            }
