$(function () {
    $("#form1").form('load', '/ashx/Contract/reviewContractData.ashx?module=LoadManageReviewApp&contractNo=' + contractNo);
    //绑定数据
    bindUI();
    //tabs选中事件
    selectTabs();
    ...
    $('#tt').tabs({
        ...
        onSelect: function (title) {
            ...
            if (title == "合同预览") {
                ...
                var language = "中文";
                $("#previewFormFrame").attr('src', '/Bus/ContractCategory/previewManageContract.aspx?language=' + language + '&contractNo=' + $("#contractNo").val());
            }
            if (title == "审核日志") {
...
function reviewSub() {
    ...
    var status = status;
    $.post("/ashx/Contract/reviewContractOperater.ashx?module=reviewManageData" + "&isapprove=" + data + "&log=" + log + "&contractNo=" + $("#contractNo").val() + "&contractStatus=" + status, function (data) {
        if (data.sucess == "1") {
...
function bindCostCategory() {
    $('#costCategoryTable').datagrid({
        ...
        columns: [[
		            ...
		            { field: 'project', title: '项目', width: 100, align: 'center' },
