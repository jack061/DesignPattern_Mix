...
namespace RM.Busines.DAL
{
    ...
    public class RM_UserInfo_Dal : RM_UserInfo_IDAO
    {
        ...
        public Hashtable GetUserInfoInfo(string key)
        {
            Hashtable ht = new Hashtable();
            var dt= GetUserInfoInfo(new StringBuilder( " and user_id=@user_id "), new SqlParam[] { new SqlParam("@user_id",key) }.ToList());
            if (dt!=null && dt.Rows.Count > 0)
            {
                ht.Add("user_id", dt.Rows[0]["user_id"]);
                ht.Add("user_name", dt.Rows[0]["User_Name"]);
            }
            return ht;
        }
