...
(function($){
    ...
    BootstrapSelect.prototype.createSelect = function(){
    	...
    	//获取元素宽、高
    	var width = $input[0].clientWidth + 4;
    	var height = $input[0].clientHeight + 3;
    	var name = $input.attr('name');//name属性
    	$input.removeAttr('name');//删除name属性，最终把name属性移到隐藏域上
    	$input.wrap('<span style="display:block;overflow:hidden;position:relative;width:'+width+'px;"></span>');
    	$input.css({'padding-left':'17px'},{'position':'relative'});
    	...
    	$('<a href="javascript:;" style="width:20px;height:'+height+'px;'
    			+'top: 0px; right: 0px;position:absolute;'
    			+'background:url(\'bootstrap/plus/images/menu_arrows.png\') no-repeat 0 center;">'
    	+'</a>').insertAfter($input);
    ...
    BootstrapSelect.prototype.createDownList = function(data){
    	...
    	var width = $this.$el[0].clientWidth + 2;
    	...
    	$('<div style="'+style+'" id="'+_id+'"></div>').appendTo('body');
    	var $div = $('#'+_id);
	...
	function setValues(bootstrapSelect,value){
		...
		$this.$contentDownList.find('div').each(function(index){
			...
			item.removeClass('selected');//清除原来选择
			//设置选中
    ...
    function makeContAndShow(inputDownList,isReload){
		...
		for(var i=0;i<data_.length;i++){
			itemsHtml += '<div style="padding-left:17px;width:100%;height:20px;">' + data_[i][options.textField] + '</div>';
		}
		$this.html(itemsHtml);
