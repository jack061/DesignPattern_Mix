...
(function ($) {
    ...
    $.fn.bootstrapTable.methods.push('getColumns',
        'getModiDatas', 'removeRow', 'getColTotal', 'getThis');
    ...
    function initTrClick(that, _this) {
        ...
        var innerText = $td[0].innerText == '-' ? '' : $td[0].innerText;
        var value = rowData[tdOpt.field] == null || rowData[tdOpt.field] == '' ? innerText : rowData[tdOpt.field];
        ...
        if (!tdOpt.edit.type || tdOpt.edit.type == 'text') {
            ...
            $(".bootstrap-select").attr("readOnly", true);
        }
    ...
    function recover(that) {
        ...
        if (that.prevEditRow != null) {
            ...
            that.prevEditRow = null;
            that.$body.find('> tr').removeClass('editable-select');
            //变更js,为调用pname文本框失去焦点事件
            if (that.data[0].nodename != undefined) {
                nodenameblur(that);
            }
        }
    }
